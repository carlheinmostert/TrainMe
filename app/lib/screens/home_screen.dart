  ...
  Widget _buildBody() {
    ...
    return Column(
      children: [
        ...
        // Bootstrap-failure banner. Normally hidden (SizedBox.shrink).
        // Surfaces when AuthService.ensurePracticeMembership failed — the
        // user otherwise has no on-screen signal that publishing will fail
        // because they have no practice membership. Tapping Retry re-runs
        // the RPC and clears the notifier on success.
        ValueListenableBuilder<String?>(
          valueListenable: AuthService.instance.bootstrapError,
          builder: (context, error, _) {
            if (error == null) return const SizedBox.shrink();
            return _BootstrapErrorBanner(
              onRetry: () => AuthService.instance.ensurePracticeMembership(),
            );
          },
        ),
...
/// Thin banner shown at the top of the session list when the bootstrap
/// RPC (`AuthService.ensurePracticeMembership` →
/// `bootstrap_practice_for_user`) failed. Without this, a
/// network/RLS/auth hiccup during first sign-in would leave the user with
/// no practice membership and no on-screen explanation of why the publish
/// button later silently refuses to work.
///
/// Dark surface, coral left-rail + icon, Montserrat headline, Inter body —
/// matches the brand banner style used elsewhere on the screen.
class _BootstrapErrorBanner extends StatelessWidget {
  final VoidCallback onRetry;

  const _BootstrapErrorBanner({required this.onRetry});
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            // Coral left-rail for visual weight without shouting.
            Container(
              width: 4,
              height: 56,
              decoration: BoxDecoration(
