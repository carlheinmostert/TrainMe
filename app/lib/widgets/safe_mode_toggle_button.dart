...
/// First-class Safe Mode toggle for the Clients screen header.
///
...
///    coral-filled icon WITH a small lock glyph in the corner. The
///    polygon enforcement means the practitioner cannot override —
///    tapping shows a brief inline snackbar "Enforced by your current
///    premises" instead of mutating state. The privacy promise is held
///    tight (see also the Switch in `plan_settings_sheet.dart`'s old
///    Safe Mode section, retired in this same PR).
///
...
///
/// Used by the Home (Clients) screen header next to the network-share
...
class SafeModeToggleButton extends StatelessWidget {
  ...
  Widget _buildIcon({required bool isActive, required bool isAutoLocked}) {
    ...
    // Active: coral-filled shield + figures. Wrapped in a coral-border
    // pill so the button reads as a "toggled on" state.
    final iconAndLock = Stack(
      clipBehavior: Clip.none,
      children: [
        SafeModeIcon(size: iconSize),
        if (isAutoLocked)
          Positioned(
            // Bottom-right corner of the icon, just outside the shield
            // so the lock glyph reads as an authority badge rather than
