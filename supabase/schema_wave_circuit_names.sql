-- Wave Circuit-Names — practitioner-editable circuit labels.
--
-- Until this wave a circuit was identified to the practitioner only by an
-- auto-assigned letter ("CIRCUIT A", "CIRCUIT B"…) derived from its
-- first-appearance order in the exercise list. The Studio header had a
-- TextEditingController wired to it that silently dropped input — the
-- comment in `circuit_control_sheet.dart` flagged this as "post-MVP, needs
-- a plans.circuit_names JSONB column". This is that column.
--
-- Shape: `{ "<circuit_id>": "<custom name>" }` — same map idiom as
-- `circuit_cycles`. Missing key = use the auto label. Empty string =
-- treat as missing (the Flutter inline-edit widget rejects empty
-- submissions, so this is mostly defensive).
--
-- Round-trip path:
--   1. Studio inline-edit -> Session.circuitNames -> SQLite
--      (sessions.circuit_names TEXT, schema v36).
--   2. Publish -> upload_service.upsertPlan({'circuit_names': map}) ->
--      plans.circuit_names JSONB.
--   3. get_plan_full returns it via to_jsonb(plan_row) automatically; the
--      web player keys off plan.circuit_names[circuit_id].
--
-- get_plan_full needs no edit (it emits to_jsonb(plan_row) which picks up
-- every column on plans automatically — that covers the web-player path).
-- list_practice_plans IS hand-built, so we re-create it below with
-- circuit_names threaded into its per-plan jsonb_build_object.

ALTER TABLE public.plans
  ADD COLUMN IF NOT EXISTS circuit_names jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.plans.circuit_names IS
  'Practitioner-set custom labels per circuit_id. Map of circuit_id -> '
  'display name. Missing key falls back to the auto-assigned letter '
  '("Circuit A" etc.) at the surface. Mirrors the Flutter SQLite '
  'sessions.circuit_names TEXT column (schema v36).';

-- ---------------------------------------------------------------------------
-- list_practice_plans — add circuit_names to the per-plan jsonb payload.
--
-- Per the schema-migration discipline rule
-- (feedback_schema_migration_column_preservation.md): when re-creating an
-- RPC, carry forward EVERY existing field on the jsonb. Diff vs the live
-- body in `schema_pull_practice_plans_with_media.sql` is one line: the
-- new `'circuit_names'` key on the per-plan jsonb_build_object.
--
-- get_plan_full needs no edit — it emits `to_jsonb(plan_row)` which
-- automatically includes every column on `plans` (so circuit_names flows
-- through for the web player without further plumbing).
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.list_practice_plans(
  p_practice_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = public, auth
AS $fn$
DECLARE
  v_caller    uuid    := auth.uid();
  v_is_owner  boolean := public.user_is_practice_owner(p_practice_id);
  v_is_member boolean := p_practice_id = ANY(ARRAY(SELECT public.user_practice_ids()));
  v_plans     jsonb;
BEGIN
  IF v_caller IS NULL THEN
    RAISE EXCEPTION 'list_practice_plans requires an authenticated caller'
      USING ERRCODE = '28000';
  END IF;

  IF p_practice_id IS NULL THEN
    RAISE EXCEPTION 'list_practice_plans: p_practice_id is required'
      USING ERRCODE = '22023';
  END IF;

  IF NOT v_is_member AND NOT v_is_owner THEN
    RAISE EXCEPTION 'list_practice_plans: caller % is not a member of practice %',
      v_caller, p_practice_id
      USING ERRCODE = '42501';
  END IF;

  WITH visible_plans AS (
    SELECT p.*
      FROM plans p
     WHERE p.practice_id = p_practice_id
       AND p.deleted_at IS NULL
       AND (
         v_is_owner
         OR NOT EXISTS (
              SELECT 1 FROM plan_issuances pi
               WHERE pi.plan_id = p.id
            )
         OR EXISTS (
              SELECT 1 FROM plan_issuances pi
               WHERE pi.plan_id    = p.id
                 AND pi.trainer_id = v_caller
            )
       )
  ),
  plan_exercises AS (
    SELECT
      e.plan_id,
      jsonb_agg(
        jsonb_build_object(
          'id',                  e.id,
          'position',            e.position,
          'name',                e.name,
          'media_url',           e.media_url,
          'line_drawing_url',    CASE
                                   WHEN e.media_type = 'rest' THEN NULL
                                   ELSE e.media_url
                                 END,
          'thumbnail_url',       e.thumbnail_url,
          'media_type',          e.media_type,
          'notes',               e.notes,
          'circuit_id',          e.circuit_id,
          'include_audio',       e.include_audio,
          'created_at',          e.created_at,
          'preferred_treatment', e.preferred_treatment,
          'prep_seconds',        e.prep_seconds,
          'start_offset_ms',     e.start_offset_ms,
          'end_offset_ms',       e.end_offset_ms,
          'video_reps_per_loop', e.video_reps_per_loop,
          'aspect_ratio',        e.aspect_ratio,
          'rotation_quarters',   e.rotation_quarters,
          'body_focus',          e.body_focus,
          'rest_seconds',        e.rest_seconds,
          'sets',                COALESCE(
                                   (SELECT jsonb_agg(
                                             jsonb_build_object(
                                               'position',                s.position,
                                               'reps',                    s.reps,
                                               'hold_seconds',            s.hold_seconds,
                                               'weight_kg',               s.weight_kg,
                                               'breather_seconds_after',  s.breather_seconds_after
                                             )
                                             ORDER BY s.position
                                           )
                                      FROM public.exercise_sets s
                                     WHERE s.exercise_id = e.id),
                                   '[]'::jsonb
                                 )
        )
        ORDER BY e.position
      ) AS exercises
    FROM exercises e
    JOIN visible_plans vp ON vp.id = e.plan_id
    GROUP BY e.plan_id
  ),
  latest_issuance AS (
    SELECT DISTINCT ON (pi.plan_id)
           pi.plan_id,
           pi.trainer_id  AS last_trainer_id,
           pi.issued_at   AS last_issued_at
      FROM plan_issuances pi
      JOIN visible_plans vp ON vp.id = pi.plan_id
     ORDER BY pi.plan_id, pi.issued_at DESC
  )
  SELECT COALESCE(
           jsonb_agg(
             jsonb_build_object(
               'id',                              vp.id,
               'practice_id',                     vp.practice_id,
               'client_id',                       vp.client_id,
               'client_name',                     COALESCE(c.name, vp.client_name),
               'title',                           vp.title,
               'circuit_cycles',                  vp.circuit_cycles,
               'circuit_names',                   vp.circuit_names,
               'preferred_rest_interval_seconds', vp.preferred_rest_interval_seconds,
               'created_at',                      vp.created_at,
               'sent_at',                         vp.sent_at,
               'version',                         vp.version,
               'first_opened_at',                 vp.first_opened_at,
               'last_opened_at',                  vp.last_opened_at,
               'deleted_at',                      vp.deleted_at,
               'crossfade_lead_ms',               vp.crossfade_lead_ms,
               'crossfade_fade_ms',               vp.crossfade_fade_ms,
               'unlock_credit_prepaid_at',        vp.unlock_credit_prepaid_at,
               'last_published_at',               li.last_issued_at,
               'last_trainer_id',                 li.last_trainer_id,
               'exercises',                       COALESCE(pe.exercises, '[]'::jsonb)
             )
             ORDER BY COALESCE(li.last_issued_at, vp.created_at) DESC NULLS LAST, vp.id
           ),
           '[]'::jsonb
         )
    INTO v_plans
    FROM visible_plans vp
    LEFT JOIN clients          c  ON c.id     = vp.client_id
    LEFT JOIN latest_issuance  li ON li.plan_id = vp.id
    LEFT JOIN plan_exercises   pe ON pe.plan_id = vp.id;

  RETURN jsonb_build_object('plans', v_plans);
END;
$fn$;

REVOKE ALL ON FUNCTION public.list_practice_plans(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.list_practice_plans(uuid) TO authenticated;

-- Smoke test (run manually after apply):
--
--   SELECT column_name, data_type, is_nullable, column_default
--     FROM information_schema.columns
--    WHERE table_schema = 'public'
--      AND table_name = 'plans'
--      AND column_name = 'circuit_names';
--
--   -- Round-trip: set a name on a real plan and confirm get_plan_full
--   -- echoes it on the wire.
--   UPDATE public.plans
--      SET circuit_names = jsonb_build_object('<circuit-id>', 'Push Day')
--    WHERE id = '<plan-id>';
--   SELECT public.get_plan_full('<plan-id>'::uuid)->'plan'->'circuit_names';
