...
stateDiagram-v2
    ...
    CLASSIFIED --> BACKLOG: enhancement accepted but blocked on a prerequisite
...
**BACKLOG** is accepted-but-not-scheduled work — real, no blocker, nobody's ball.
You set it by dragging a card to the `Backlog` column; the bot can also file an
enhancement there during triage when it's clearly wanted but **blocked on a
prerequisite** (needs data first, or another issue must land first). It's distinct
from **HOLD** (parked for a *specific reason* — a pending decision or block, always
Carl-set). Both are **skipped by the sweep**, and neither sits in "Needs you".
...
- **Columns:** `Triage` · `Needs you` (decisions) · `Go` · `Building` · `To merge` · `To test` · `Backlog` · `Hold` · `Done`.
...
- **Dragging a card to `Go` is the universal "approve / advance" — you never pick
  ...
  - → `Hold` = park
  - → `Backlog` = accepted but not scheduled (no blocker; bot un-assigns)
  - → `Done` = close outright
