...
- **Labels** (status track): `status:needs-info`, `status:awaiting-design-approval`,
  ...
  `status:awaiting-merge`, `status:awaiting-validation`, `status:hold`,
  `status:backlog` — plus the native `bug`, `enhancement`, `help wanted`.
- **Board**: a Projects v2 board with a single-select **"Stage"** field whose
  ...
  `To test` · `Backlog` · `Hold` · `Done`. (`Go` is a *command* column — Carl drags an item there to approve/advance
...
for L in "status:needs-info|fbca04|Bot is waiting on the issue author for more detail" \
         ...
         "status:hold|e4e669|Bot: hands off this issue" \
         "status:backlog|c2c8d0|Accepted, not scheduled — parked in the backlog"; do
   ...
   mutation {
     createProjectV2Field(input:{
       ...
       singleSelectOptions:[
         ...
         {name:"Backlog",   color:GRAY,   description:"Accepted, not scheduled — parked in the backlog"},
...
- **Set its Stage column from its current labels** (canonical map): a `status:*`
  ...
  any `awaiting-*` → Needs you; `status:hold` → Hold; `status:backlog` → Backlog;
  **no status label → Triage.**
...
merge / To test / Backlog / Hold / Done) with cards draggable between them. (Cards also show GitHub's built-in
