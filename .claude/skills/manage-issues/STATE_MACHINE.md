...
| **HOLD** | `status:hold` | Carl (manual) | "Bot, hands off" — parked for a reason (Stage `Hold`) |
| **BACKLOG** | `status:backlog` | nobody | Accepted but not scheduled — no blocker (Stage `Backlog`); Carl-dragged or bot-filed; always skip |
...
| CLASSIFIED | enhancement accepted but blocked on a prerequisite | file to backlog, no assignee | BACKLOG |
...
| *any* | drag to `Backlog` from Carl | add `status:backlog`, un-assign | BACKLOG |
| HOLD | always | skip | HOLD |
| BACKLOG | always | skip | BACKLOG |
