...
- **Skip parked issues:** `wontfix` / `duplicate` / `invalid` / `status:hold` / `status:backlog`. Skip PRs.
...
| **BACKLOG** | `status:backlog` | nobody | Backlog |
...
2. **Consume command columns first** (Carl's inputs — these are read *before* state logic): for every card whose **Stage** is a command value, treat it as that command and act, then move the card to the resulting real-state column:
   ...
   - **`Backlog`** → add `status:backlog` → Backlog column (accepted, not scheduled; un-assign — nobody's ball).
...
1. **`status:hold` / `status:backlog` / `wontfix` / `duplicate` / `invalid`** → skip.
...
10. **No `status:` label (TRIAGE / CLASSIFIED)**:
    ...
    - **Enhancement**: clearly accepted/desirable but **blocked on a prerequisite** (needs data first, or another issue/feature must land first) → file to **Backlog** (`status:backlog`, no assignee) — keeps *Needs you* for decisions due now. Otherwise, enough detail → post the **design spec**, `status:awaiting-design-approval`, assign **Carl**. Too little → grill, `status:needs-info`, assign author.
...
- **Mirroring over a card sitting in `Go` / `Hold` / `Backlog` / `Done`** → never. Those are Carl's *pending commands*, consumed in board-sync **step 2**; the **step 3** mirror only touches *non-command* cards. Overwriting a Go/Hold/Backlog/Done card silently discards his input (a re-sync that loops every card must respect this).
- **Confusing Backlog with Hold** → Hold = parked for a *reason* (a pending decision/block, Carl-set); Backlog = accepted but *not scheduled* (no blocker; the bot may file it). Both are skipped by the sweep. Don't mirror a card out of either.
