...
- [ ] 13. Take a photo with no faces visible (everyone facing away / looking down). Expected: the conversion is rejected via `SafeModeRejection`, the exercise row is discarded, and the coral-bordered toast appears at the top of the viewfinder (Vision found no faces so the v2 pipeline reports `missRate=1.0`, which exceeds `kSafeModeMaxMissRate=0.05`).
