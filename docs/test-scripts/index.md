...
- [2026-05-28 — Unpublished-changes coral spine](2026-05-28-exercise-change-marker.md) — new per-exercise + session-level "dirty since publish" marker. Coral 4px left-edge spine on Studio exercise cards (per-exercise `last_edited_at`, SQLite 49->50) AND session-list cards (existing `hasUnpublishedContentChanges` signal), one shared `ChangeSpine` widget. Reorder lights EVERY exercise (Carl's reversal of the spec draft). Mobile-only, no web-player parity. 20 items across migration sanity / edit-site coverage / reorder / clear-on-publish / offline / web-player-stays-clean.
- [2026-05-27 — Staging tip full QA — device install](2026-05-27-staging-tip-full-qa.md) — staging tip `dc518d3`. 30 ios-impact PRs since `f69a6c3`: M40 ARKit pivot, M41 self-recognition diagnostic, pose-HUD restore, M42 tolerance decouple, M42b yaw sign flip, artifact-system Waves 1-5, code-review owner-only RPC fix #544, home polish, artifact-stacking UI #548. 52 items across pre-flight + face enrolment + Studio deck + /me bundles + brand-skin + share sheet + handouts + polish + regression + known gaps. **Supersedes** `2026-05-27-artifact-stacking.md` (29-item subset bundled in as group C + D).
