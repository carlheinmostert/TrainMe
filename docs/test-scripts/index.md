...
- [2026-05-28 — Handout "Loading your plan" hang — true root cause (CSS)](2026-05-28-handout-loading-css-rootcause.md) — the three prior rounds (config.js ordering, watchdog, `?v={sha}`) all missed: the JS was working. Real cause was `handout.css` `display: flex` on `.handout-loading`/`.handout-page`/`.handout-error` defeating the global `hidden` attribute, so the loading overlay never un-hid. One-line fix `[hidden] { display: none !important; }`. Confirmed on simulator (render completed, fetch 200, yet overlay stayed). 8 items.
