...
| Miss rate (v2 video definition — see below) | Outcome |
|---|---|
| 0% ≤ miss ≤ 5% | Accept with safe variant |
| 5% < miss < 100% | Reject (catastrophic detection failure — both Vision modalities returned nothing) |
| 100% (exactly) | Accept as no-PII (no humans in any frame) |

This rule transfers to v2 video verbatim, but the definition of "miss" is intentionally tighter than v1's:

- v1 video defined a "miss frame" as one where **Vision face detection returned zero detections**. A single-signal definition.
...
**Decision: v2 video redefines "miss" as "Vision detected zero humans AND person segmentation returned no positive pixels" — both signals must fail.** This is intentionally tighter than v1's face-detect-only definition. Legitimate backlit captures (silhouette present but face hidden) and back-to-camera sections (face hidden, silhouette sharp) no longer reject — segmentation still fires, the AND term keeps the frame out of the miss bucket. The 5%–100% rejection band now triggers only on catastrophic detection failures where both Vision modalities fail simultaneously (e.g., severely overexposed frames, frames where the subject is fully outside the visible spectrum, or a fully-blocked lens).

Frames where Vision found humans (face or segmentation) but no face matched the subject are NOT misses — they correctly produce no-subject-mode output (all detected non-matching faces blurred, scenery sharp, per the v2 photo rule on no-subject frames). This preserves the existing fail-closed semantics: the rule catches catastrophic detection failures, not subject-absence cases.

A 30-second video of an empty room (or a video where the practitioner is permanently outside both modalities' reach) hits 100% miss and is accepted as no-PII. A 30-second video where Vision intermittently loses both face AND silhouette across more than 5% of frames hits the middle band and is rejected with the same "couldn't track everyone — try a different angle" copy from the zero-detection spec. Backlit + back-to-camera captures, which silhouette through but lose the face, are accepted with safe variant — they run through no-subject mode for the face-hidden sections.
...
#### Re-confirm tracker-drop softening — 3-consecutive-miss rule

The re-confirm cadence (every M seconds, M=2) runs fresh face-rec on a single frame to verify the tracker is still on subject. A naive implementation drops the tracker the moment that single frame fails to match — but a single frame can fail for benign reasons (motion blur during a rep, eyes closed, brief head turn, transient lighting change) while the tracker is independently still holding high confidence on the same physical face.

**Decision: tolerate up to 3 consecutive re-confirm misses before dropping the tracker, provided the tracker's own confidence is still ≥ `trackerConfidenceFloor`.**

Specifically:
- Maintain a `consecutiveReConfirmMisses` counter per video on the state machine.
- On a re-confirm where face-rec matches: reset the counter to 0; proceed with the existing confirm-or-re-seed logic (keep tracking if the matched face is within `reSeedProximityRadiusFrac × frame.height`, else re-seed).
- On a re-confirm where face-rec misses BUT the tracker's prior-frame confidence was still ≥ `trackerConfidenceFloor`: increment the counter; if it's < 3, KEEP the tracker and stay in `.tracking`; if it's ≥ 3, fall through to the existing drop-tracker behaviour (clear `trackObservation`, transition to `.noSubject`).
- Independent tracker-confidence drops (the `.tracking` branch detects confidence < floor) are unaffected and still transition immediately to `.lost`, regardless of the counter — those represent the tracker itself losing the lock, which is a stronger signal than a single bad face-rec frame.

Rationale: section 6e explicitly says "trust the tracker on the existing subject"; the naive implementation contradicts that. The 3-frame tolerance preserves subject continuity across realistic capture artefacts (a typical 30-fps clip with a 2-second re-confirm cadence means 3 misses span 6 seconds — long enough that a genuine subject-loss is well separated from a single-frame artefact).
...
8. **Heavily backlit video — accepted with segments.** Record practitioner against a bright window; Vision intermittently loses face detection but person segmentation stays visible across the silhouette. Result: capture accepted with safe variant (the v2 "miss = face AND segmentation both fail" rule keeps these frames out of the miss bucket); face-hidden sections run in no-subject mode for those frames; no rejection toast. Carl's earlier expectation (v1 rejection) is intentionally retired — see section 6b for the redefinition. To exercise the actual rejection path now, point the lens at a fully-obscured surface (palm over lens, lens cap on) so BOTH modalities fail — that still middle-bands and rejects with the "couldn't track everyone" toast and no orphan exercise card (orphan fix from the zero-detection spec must still hold for video).
