...
- **Database (Supabase)** — migration filename timestamp is the version. Timestamp-named files under `supabase/migrations/` (`YYYYMMDDHHMMSS_<name>.sql`) are the canonical chain Supabase Branching applies to every per-PR DB. Older ad-hoc patch files under `supabase/archive/` predate the baseline migration (2026-05-11) and are historical reference only — do not apply them. The local SQLite mirror has its own `app/lib/services/local_db.dart` `_dbVersion` integer that bumps on every column-add / table-add migration.
...
- **Backend:** Supabase (yrwcofhovrcydootivjx.supabase.co). **CLI is linked** and **Supabase Branching is enabled** (since 2026-05-11) — schema changes ship as timestamp-named files under `supabase/migrations/`. Per-PR DB previews run automatically; merging to `staging` applies on the persistent staging branch (`vadjvkmldtoeyspyoqbx`); merging staging → main applies on prod. Direct `supabase db query --linked --file` is still available for hot-fixes outside the migration chain but should be a last resort.
...
- **`docs/CHECKPOINT_2026-05-25-self-trainer-wave-shipped.md` — READ FIRST.** The self-trainer wave is now end-to-end on staging. The morning [foundation wave](docs/CHECKPOINT_2026-05-25-self-trainer-foundation.md) landed schema + IA + face-embedding + plan_artifacts + privacy docs + CI fix. The completion wave that followed landed the practitioner-facing surface and billing plumbing: public-profile consent UI with lazy Self-client backfill (#502), in-app migration banner (#501), Safe Mode subscription gate + paywall + portal subscribe page (#504), capture-time self-verification stamping (#503), publish cost preview with conditional free path on `consume_credit` (#507), and the My Workouts body with FAB-driven capture entry (#508). Twelve PRs total across both waves, all merged to staging same UTC day. SQLite v47 → v48. Three new gotchas: sub-agent watchers print-don't-merge, `gh pr merge` returns 0 against already-merged PRs, sub-agent briefs go stale fast — always pre-flight against the live DB. Staging tip `f47d7f7`, main tip `4bc7347` (docs-only ahead). iPhone CHM still pre-wave — Carl said "test at the end" so wave-close install is the next ask.
- `docs/CHECKPOINT_2026-05-25-self-trainer-foundation.md` — Self-trainer foundation: six PRs landing schema + IA rename + face-embedding native + plan_artifacts write + biometric privacy docs + pgvector CI fix. ADRs 0020/0021/0022 + design doc `docs/SELF_TRAINER_WAVE.md` ratified the 14-decision wave before code. Decisions made autonomously while Carl was away: detached worktrees for docs-direct-to-main, MobileFaceNet 192-d (later corrected to 512-d in PR #494), `safe_mode_month_trial` as the trial-kind name, 4-credit subscription price from Q7.4c.
- `docs/CHECKPOINT_2026-05-25.md` — Same-day Safe Mode v2 polish wave (six PRs riding beneath the self-trainer wave). Three bug-fix PRs in the morning closed the v2-on-photos sev1 chain (PR #482 DeviceRGB colorspace fix, PR #485 algo-version bump, PR #489 CachedClient embedding 2048-byte fix). Three feature PRs in the afternoon hardened surrounding UX (PR #490 zero-detection accept, PR #491 enrolment phase 1, PR #496 enrolment phase 2). Dead-agent salvage event (PR #496's implementation agent died silently with 3,700 lines uncommitted — salvaged with zero modifications). v2-on-video spec parked at `docs/specs/2026-05-25-safe-mode-v2-video.md`.
- `docs/CHECKPOINT_2026-05-24.md` — Two-day wave (2026-05-22 → 2026-05-23) of ~60 PRs on staging: Safe Mode Transparency wave (public per-premises live view at `/v/{practice}/{premises}/now` with Leaflet+Esri map, poster generation, identity gate), Safe Mode v2 face-recognition discriminator (on-device MobileFaceNet replaces "largest bbox = client" heuristic — first attempt crashed selfie flow, reverted, then properly hardened across ~8 follow-up PRs), capture audit infrastructure (per-capture audit events table + anon roster RPC + 24h drawer on live page), portal cosmetic pass (coral `.studio` lockup, Classes coming-soon tile, Credits-merges-Network, premises inline page restoring R-01), CI quality-of-life (eager SW update on register + visibilitychange, iOS-skip on web-mirror PRs + `ios-impact` auto-label + `tools/ios-pending-prs.sh`). Mapbox→Leaflet+Esri reversal lesson encoded. Supabase Branching wedged on staging since 2026-05-11 — handoff at `docs/handoffs/2026-05-23-safe-mode-embedding-and-branching-reconciliation.md`.
- `docs/CHECKPOINT_2026-05-21-coordination.md` — Coordination checkpoint mid-wave: V2 sub-agent in flight, Safe Mode completion queued. 10 PRs on the day (Safe Mode Phase 1+2 + premises editor unblocks + map polish). Useful for the decisions table (brand-color scope, public-profile portal IA, map address picker stack, Safe Mode fail-closed threshold).
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`), sev1 portal hotfix for an env.ts Webpack-inline bug, backport to staging via PR #388, and TestFlight `1.0.0+4` queued at Apple. Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access; bracket-notation `process.env[name]` resolves to `undefined` in the browser. Main tip `1cb555e`, staging tip `b219370`.
- `docs/CHECKPOINT_2026-05-17.md` — Morning checkpoint: device QA wave; four bugs fixed same-day. Photo B&W `pickPrimarySrc` parity (PR #381), video swap-back re-hydrate (PR #382), four-PR cache stack with URL versioning `?v=<plan.version>` as the spine (PR #385), settings popover escaped the `backdrop-filter` containing-block trap (PR #386).
- `docs/CHECKPOINT_2026-05-16.md` — Publish unblocked (raw-archive RLS recovery PR #369), diagnostic surface keystone (PR #362 + #366 architectural rewrite), SW network-first for app shell (PR #372), hero-crop two-part fix (auto-pick from segmentation centroid + manual drag override, PR #375), thumb-republish-on-regen via `thumbnailsDirty` SQLite flag (PR #376). 14 PRs merged. iPhone CHM ended on `tmp/three-fixes-for-device` build.
- `docs/CHECKPOINT_2026-05-15.md` — Publish-flow refactor (PR-A/B/C) + Supabase storage policies recovery + 9 fix waves. 20 PRs merged to staging. iPhone CHM ended on staging tip `6d3444b`. Three load-bearing decisions ratified: B&W primary treatment, publish is atomic, consent decouples from upload. Circuit attempt #10 mockup-locked (variant 1 picked).
- `docs/CHECKPOINT_2026-05-11.md` — CI/CD release-train cutover. 12 PRs landed the three-tier pipeline (feature → staging → main), Supabase Branching live on prod, Vercel-Supabase integration on both web projects, `staging.*` DNS at Hostinger, Flutter `ENV` flag, `release-tag.yml` auto-tag workflow, 72 legacy `schema_*.sql` files archived to `supabase/archive/`. 8 new memory rules.
- `docs/CHECKPOINT_2026-05-04.md` — Big polish + diagnostics day: 27 PRs (#196 → #223) including the hold-position 3-mode picker (Wave 43), inline-editable circuit names, settings-sheet tabs, Hero-frame static thumbnails, filmstrip session-card backgrounds. Two Supabase migrations applied. SQLite v33 → v37. Diagnostic tooling: orphan-agent detection hooks (#201) + long-press conversion-error log reader (#213).
- `docs/CHECKPOINT_2026-05-01.md` — Per-set PLAN post-merge QA: 8 stacked PRs (#149-#156). Obsoleted by the 2026-05-04 wave on top.
...
**Shipped 2026-05-25 (Safe Mode v2 polish day — 6 PRs #482, #485, #489, #490, #491, #496):**
- **PR #482 — Safe Mode v2 photo colorspace fix.** `fix(safe-mode-v2): photo render uses DeviceRGB colorspace — kills dark + whole-frame-blur`. The Safe Mode photo render was passing `colorSpace: nil` against a default CIContext; CoreImage emitted linear-light bytes that the JPG encoder treated as gamma-encoded sRGB, producing ~30% too-dark photos with a whole-frame blur appearance. Fix: pass `CGColorSpaceCreateDeviceRGB()` explicitly. Bench evidence (brightness within 0.4% of source, sharpness ratio 1.02) at `/tmp/safe-mode-bench-iter-1.html`. Sev1 that blocked the V2 photo path from being the canonical pipeline.
- **PR #485 — Bump Safe Mode v2 algo version to 3.** `fix(safe-mode-v2): bump algo version to 3 — surfaces re-process row for existing v2 captures`. Single-line bump that re-surfaces the "Re-process Safe Mode" row in the editor sheet for every existing v2-captured dark photo so Carl could regenerate his stock through the corrected (#482) pipeline. The Dart-side reprocess wiring was already correctly multi-ref from Wave-E — only the version constant was missing. Closed the loop on #482.
- **PR #489 — CachedClient embedding decoder units fix.** `fix(safe-mode-v2): CachedClient embedding decoder expects 2048 bytes not 512`. Units bug: both Dart decoders in `CachedClient` (cloud-JSON + SQLite-row paths) checked `bytes.length != 512`, but MobileFaceNet outputs 512 floats × 4 bytes = 2048 bytes. Every sync silently dropped the embedding → cold-start hydration never primed → the capture screen showed "Prepare a face fingerprint" CTA for already-enrolled clients. Fix: introduced `kFaceEmbeddingBytes = 2048` constant + fixed both decoders + corrected 3 drifted doc comments + added 12 regression tests (one explicitly asserts the broken 512-byte shape is now REJECTED). Without this, Wave-E's multi-ref enrolment work couldn't cold-start a real client. Motivated [`gotcha_face_embedding_units.md`](../.claude/projects/-Users-chm-dev-TrainMe/memory/gotcha_face_embedding_units.md).
- **PR #490 — Accept zero-detection captures + telemetry + orphan-row fix.** `fix(safe-mode): accept zero-detection captures + telemetry + orphan-row fix`. Three changes bundled per Carl's "all in one PR" ask: (1) captures where Vision detected zero humans (landscape, equipment, empty room) are now accepted instead of rejected (single unified rule across photos + videos); (2) every accepted-empty event writes a `safe_mode.accepted_empty` row to `capture_audit_events` with a scene fingerprint (mean RGB, entropy, complexity score — no image bytes); required relaxing a CHECK constraint on the `kind` column; (3) orphan-exercise-after-rejection bug — Studio + ClientSessions had no removal stream to subscribe to, so rejection-deleted rows stayed visible until navigation; fix is a new `onExerciseRemoved` stream both screens subscribe to.
- **PR #491 — Enrolment polish phase 1.** `feat(safe-mode-v2): enrolment polish phase 1 — camera toggle + consent sheet + consent-aware mode scaffolding`. Six concerns: camera flip toggle on enrolment screen (sticky per-device); `FaceEnrolmentMode` enum encoding the four-cell consent matrix; avatar-tap intercept + capture-banner "Set face" CTA both consent-aware; `avatarOnly` simple-mode capture (single-shot shutter, no sweep, replaces the legacy avatar-only path retired in Wave-E); consent sheet restructure (dropped "Safe Mode" section header, moved face-rec toggle into "Profile" group below avatar toggle); R-10 parity on web portal (face-rec toggle had never been wired through there); mode-resolution unit tests. **Surprise during implementation:** the consent sheet had FOUR sections (Video / Profile / Analytics / Safe Mode) not three — spec missed the Analytics section that landed in Wave 17; agent handled correctly inline.
- **PR #496 — Enrolment polish phase 2.** `feat(safe-mode-v2): enrolment polish phase 2 — pose gating + quality scoring + manual avatar`. Three concerns: real-time pose-gated capture (Vision pose stream; sweep accepts frames only if pose differs by ≥25° Manhattan from every existing slot AND quality ≥60; 6-segment guidance ring; auto-begins on face detection, no Start button per Carl's mockup signoff); per-embedding quality scoring (composite 0..100 = 30% face confidence + 25% sharpness + 20% lighting + 15% pose uniqueness + 10% embedding L2 norm; rejected slots flash rose-tinted toast showing raw score); manual avatar selection grid (post-sweep 3×2 grid of face crops + quality histogram; frontal-pick highlighted by default, tap to override; skipped entirely in `embeddingOnly` mode). Five mockup design decisions honored per Carl's signoff: 6 pose buckets (not 8), reject toast shows raw score, pose labels word-form, dashed face guide, no Start button. **Dead-agent salvage:** the implementation agent died silently with 3,700 lines of analyzer-clean, tests-passing uncommitted work after ~30 min; salvaged by authoring the missing test wave script + index entry, then committing the agent's work verbatim. Motivated [`feedback_dead_agent_salvage.md`](../.claude/projects/-Users-chm-dev-TrainMe/memory/feedback_dead_agent_salvage.md).

**Shipped 2026-05-03/04 (Wave 43 + filmstrip polish wave — 27 PRs #196 → #223):**
- **Wave 43 hold-position 3-mode** (#216) — per-set `hold_position TEXT` ENUM `('per_rep', 'end_of_set', 'end_of_exercise')` defaulting to `end_of_set`. Math branches per mode in both surfaces (web player + Flutter). 3-pill segmented control under the Hold stepper in Plan tab. Migration backfills existing rows with `hold_seconds > 0` to `'per_rep'` so already-shipped plans keep their displayed durations. Wording: "Per rep / End of set / End". SQLite v36, Supabase migration `supabase/archive/schema_wave43_hold_position.sql` applied (now part of the 2026-05-11 baseline).
- **Wave Circuit-Names** (#205) — practitioner-editable circuit labels via `plans.circuit_names jsonb` map. Studio header inline-edit. SQLite v37, Supabase migration `supabase/archive/schema_wave_circuit_names.sql` applied (now part of the 2026-05-11 baseline).
...
**Shipped 2026-04-20:**
- **Credit model overhaul (Milestone M)** — signup bonuses move to bootstrap/claim: +3 organic, +8 referred. Referrer 5% lifetime rebate with a 1-credit goodwill floor on first payout. One-time +10 bonuses removed. `supabase/archive/schema_milestone_m_credit_model.sql`.
...
- **`docs/CHECKPOINT_2026-05-25-self-trainer-wave-shipped.md`** — most recent checkpoint. Self-trainer wave shipped end-to-end on staging via twelve PRs across two waves the same UTC day. Completion wave covered the practitioner surface + billing plumbing (#501 banner, #502 consent UI + Self-client backfill, #504 Safe Mode subscription gate, #503 self-verification capture, #507 publish cost preview + `consume_credit` free path, #508 My Workouts body + FAB). SQLite v47 → v48. Three new gotchas: sub-agent watchers print-don't-merge, `gh pr merge` returns 0 against already-merged PRs, sub-agent briefs go stale fast. Staging tip `f47d7f7`, main `4bc7347`.
- `docs/CHECKPOINT_2026-05-25-self-trainer-foundation.md` — Self-trainer foundation: six PRs landing schema + IA rename + face-embedding native + plan_artifacts write + biometric privacy docs + pgvector CI fix. ADRs 0020/0021/0022 + design doc `docs/SELF_TRAINER_WAVE.md` ratified the 14-decision wave before code.
- `docs/CHECKPOINT_2026-05-25.md` — Same-day Safe Mode v2 polish wave riding beneath the self-trainer wave: six PRs (#482 photo colorspace fix, #485 algo-version bump, #489 CachedClient embedding 2048-byte fix, #490 zero-detection accept, #491 enrolment phase 1, #496 enrolment phase 2). Dead-agent salvage event on #496. v2-on-video spec parked at `docs/specs/2026-05-25-safe-mode-v2-video.md`.
- `docs/CHECKPOINT_2026-05-24.md` — Two-day wave (2026-05-22 → 2026-05-23) of ~60 PRs on staging: Safe Mode Transparency wave (per-premises live view + poster + identity gate), Safe Mode v2 face-recognition discriminator (MobileFaceNet replaces "largest bbox = client" heuristic), capture audit infrastructure (per-capture events table + 24h roster), portal cosmetic pass (coral `.studio` lockup, Classes tile, Credits-merges-Network, premises inline page restoring R-01), CI quality-of-life (eager SW update, iOS-skip on web-mirror PRs, `ios-impact` auto-label). Mapbox→Leaflet+Esri reversal lesson encoded.
- `docs/CHECKPOINT_2026-05-21-coordination.md` — Coordination checkpoint mid-wave; useful for the decisions table (brand-color scope, public-profile portal IA, map address picker stack, Safe Mode fail-closed threshold).
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`); sev1 portal hotfix for an env.ts Webpack-inline bug; TestFlight `1.0.0+4` queued at Apple. Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access.
- `docs/CHECKPOINT_2026-05-17.md` — Device QA wave; four bugs fixed same-day (photo B&W `pickPrimarySrc` parity #381, video swap-back re-hydrate #382, four-PR cache stack #383/#384/#385 with `?v=<plan.version>` URL versioning as the spine, settings popover containing-block escape #386).
- `docs/CHECKPOINT_2026-05-16.md` — Publish unblocked, diagnostic surfaces work, SW network-first for app shell, hero crop two-part fix (auto-pick + manual drag), thumb-republish via `thumbnailsDirty` SQLite flag. 14 PRs.
- `docs/CHECKPOINT_2026-05-15.md` — Publish-flow refactor (PR-A/B/C) + Supabase storage policies recovery + 9 fix waves (20 PRs merged). Three big decisions: B&W primary treatment, publish is atomic, consent decouples from upload. Circuit attempt #10 mockup-locked.
- `docs/CHECKPOINT_2026-05-11.md` — CI/CD release-train cutover: 12 PRs landed the three-tier pipeline (feature → `staging` → `main`), Supabase Branching live on prod (per-PR DB previews + persistent staging branch), Vercel-Supabase integration on both web projects, `staging.*` DNS at Hostinger, Flutter `ENV` flag, `release-tag.yml` auto-tag workflow (`v{date}.{N}`), `bump-version.sh` mobile tags (`mobile-v*`), 72 legacy `schema_*.sql` files archived to `supabase/archive/`. 8 new memory rules. Pipeline runs but doesn't gate PRs yet.
- `docs/CHECKPOINT_2026-05-04.md` — Polish + diagnostics wave (27 PRs #196 → #223 — Wave 43 hold-position, Wave Circuit-Names, settings tabs, Hero-frame thumbnails, filmstrip session cards, count glyph, uniform veil, white text). SQLite v37. Supabase: two migrations applied. Diagnostic tooling: orphan hooks + long-press conversion-error log reader.
...
- Supabase schema changes: write a timestamp-named file under `supabase/migrations/` (`YYYYMMDDHHMMSS_<name>.sql`). PR it against `staging`; Supabase Branching applies the migration to a fresh per-PR DB clone in CI. Merging to staging applies on the persistent staging branch; promoting staging → main applies on prod. Direct `supabase db query --linked --file ...` is still available for emergency hot-fixes but should be the exception, not the routine.
