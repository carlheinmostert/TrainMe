...
- **`docs/CHECKPOINT_2026-05-16.md` — READ FIRST.** Circuit attempt #10 shipped + verified (variant 1 outward ripple, pure CSS, no JS — the recurring bug is structurally gone). Hero-crop resolver centralised into `web-player/hero_resolver.js` (PR #364) — PDF object-fit squash bug fixed as a side effect; 3 enforcement layers wired (`docs/HERO_RESOLVER.md` + memory entry + `scripts/ci/check-hero-resolver.sh`). Publish-flow diagnostic surface gained a "Show error details →" fallback + atomic media-bucket uploads (PR #362, with tap-dead fix). Nine `homefit-*` skills installed for recurring workflows. iPhone CHM on staging `5e107f8` at session end. Open follow-up: Replace-pill PR — agent killed mid-edit, uncommitted partial work on branch `fix/studio-replace-pill-restore-reorder`.
...
- **`docs/CHECKPOINT_2026-05-16.md`** — most recent checkpoint. Circuit attempt #10 shipped (variant 1 outward ripple, pure CSS, no JS — the recurring bug is structurally gone); hero-crop resolver centralised into `web-player/hero_resolver.js` (PR #364) with 3 enforcement layers (`docs/HERO_RESOLVER.md` + memory + CI grep); publish-flow diagnostic surface gained "Show error details →" fallback + atomic media-bucket uploads (PR #362); 9 `homefit-*` skills installed.
