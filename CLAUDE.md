...
- **`docs/CHECKPOINT_2026-05-25.md` — READ FIRST.** One-day wave of six PRs that turned Safe Mode v2 from "shipped but bruised" into "shipped and polished". Three bug-fix PRs in the morning closed the v2-on-photos sev1 chain (PR #482 DeviceRGB colorspace fix kills dark + whole-frame-blur, PR #485 algo-version bump re-surfaces re-process row for existing v2 captures, PR #489 CachedClient embedding decoder expects 2048 bytes not 512 — units bug silently dropping every embedding through sync). Three feature PRs in the afternoon hardened surrounding UX (PR #490 accept zero-detection captures + telemetry + orphan-row fix, PR #491 enrolment polish phase 1 camera toggle + consent sheet restructure + consent-aware mode scaffolding, PR #496 enrolment polish phase 2 pose gating + quality scoring + manual avatar grid). Also: dead-agent salvage event (PR #496's implementation agent died silently with 3,700 lines uncommitted — salvaged with zero code modifications), pgvector CI hiccup unblocked by parallel-session PR #498. v2-on-video spec parked at `docs/specs/2026-05-25-safe-mode-v2-video.md`. Staging tip `d452f3e`, main tip `97e2078` (docs-only ahead). iPhone CHM still on `277e839` — next install will be produced by the parallel session atop staging.
...
- **`docs/CHECKPOINT_2026-05-25.md`** — most recent checkpoint. One-day wave of six PRs polishing Safe Mode v2 (#482 photo colorspace fix, #485 algo-version bump, #489 CachedClient embedding 2048-byte fix, #490 accept zero-detection + telemetry + orphan-row fix, #491 enrolment polish phase 1, #496 enrolment polish phase 2). Dead-agent salvage event on #496. pgvector CI hiccup unblocked by parallel-session PR #498. v2-on-video spec parked at `docs/specs/2026-05-25-safe-mode-v2-video.md`. Staging tip `d452f3e`, main `97e2078` (docs-only ahead).
