...
- **`docs/CHECKPOINT_2026-05-17.md` — READ FIRST.** Device QA wave on yesterday's staging tip; surfaced four visible bugs along the way, all fixed same-day. Photo B&W `pickPrimarySrc` patched to mirror `pickPosterSrc` (PR #381). Video swap-back gained re-hydrate so the inactive poster crops to 1:1 (PR #382). The cache strategy was "everything immutable" since day one — broke silently when PR #376 made thumbs mutable; defence-in-depth fix via SW network-first (PR #383) + SW cache:reload (PR #384) + URL versioning via `?v=<plan.version>` as the architectural spine (PR #385). Settings-popover off-screen because `#lobby-cta-bar`'s `backdrop-filter` establishes a containing block for `position: fixed` descendants — move popover to `document.body` at open (PR #386). Items 1-5 of test list pass on Mac Safari + iPhone Safari. Staging tip `0329d12`, main tip `9571dbf` (unchanged today).
...
- **`docs/CHECKPOINT_2026-05-17.md`** — most recent checkpoint. Device QA wave on yesterday's staging tip; four visible bugs surfaced live, all fixed same-day. Photo B&W `pickPrimarySrc` parity (PR #381). Video swap-back re-hydrate (PR #382). Four-PR cache-strategy stack: SW network-first (PR #383) + cache:reload (PR #384) + URL versioning `?v=<plan.version>` as the spine (PR #385). Gear popover escapes `backdrop-filter` containing block (PR #386). Plus #289 admin password reset (lingering 6 days). Items 1-5 of test list pass cross-browser. Staging at `0329d12`; main unchanged at `9571dbf`.
