...
- **Face enrolment pose source (M40, 2026-05-26)** — capability-gated split at launch. TrueDepth devices (iPhone X and newer — `ARFaceTrackingConfiguration.isSupported` returns true) get the ARKit PRIMARY path (`app/ios/Runner/FaceEnrolmentARKitChannel.swift`) which derives continuous 6-DoF head pose from `ARFaceAnchor.transform` at the camera frame rate. Non-TrueDepth devices (iPhone SE family) fall back to the Vision-on-CMSampleBuffer FALLBACK path (`app/ios/Runner/FaceEnrolmentCameraChannel.swift`). Both expose the IDENTICAL Flutter surface (MethodChannel `homefit/face-enrolment-camera` + EventChannel `homefit/face-enrolment-pose-stream` + PlatformView view-id `homefit/face_enrolment_camera_preview`), so `app/lib/services/face_enrolment_camera.dart` is agnostic to which native channel is bound. Only one channel registers per launch (Flutter's `setMethodCallHandler` is last-writer-wins per channel name). Per `feedback_no_silent_fallbacks`: if ARKit fails mid-session on a TrueDepth device, the channel surfaces an `AR_SESSION_FAILED` error to Dart rather than silently switching to Vision.
...
- **`docs/CHECKPOINT_2026-05-26.md` — READ FIRST.** Face enrolment ARKit pivot wave: 6 PRs landed `b543130` → `c347041` (staging tip). ARKit primary path on TrueDepth devices (iPhone X+) via `FaceEnrolmentARKitChannel.swift`; Vision fallback retained for iPhone SE family. Per-axis prompt-walker tolerance (yaw 20°, pitch 30°) replaces Manhattan sum. ARKit yaw sign convention: do NOT negate the YXZ extraction. Three permanent diagnostic surfaces under Settings → 7-tap → Diagnostics (pose HUD toggle, match-log toggle, "Test self-recognition" action). **One critical follow-up:** non-TrueDepth fallback channel still has the AVMetadata-bounds crop bug (cosSim ≈ 0.09 for SE users). Disk space: nuked Xcode DerivedData mid-session (14 GB recovered); ~30 GB more recoverable from agent worktree build dirs.
- `docs/CHECKPOINT_2026-05-26-artifact-system-waves-4-5.md` — Autonomous overnight run that landed Waves 4 + 5 of the artifact-system rollout (brand-skin subscription #539, share sheet + managed email #541) plus a code-review-driven follow-up PR (#544) that closed a sev1 owner-only gap on the subscription debit RPCs (brand-skin + the paired Safe Mode fix). Staging tip `597a600`. Two Carl-side actions block E2E: Supabase Auth redirect allowlist for the `/me` claim flow (Wave 2), and `RESEND_API_KEY` + `supabase functions deploy send-artifact-email` for the managed-email path (Wave 5). All 5 artifact-system waves are now staging-only pending the next staging → main promotion.
- `docs/CHECKPOINT_2026-05-25.md` — Safe Mode v2 polish day (six PRs + parallel enrolment work). Three bug-fixes closed the v2-on-photos sev1 chain (colorspace, algo-version, embedding-decoder units). Three feature PRs hardened the surrounding UX (zero-detection accept + telemetry + orphan-row fix; enrolment polish phases 1 + 2 with pose gating + quality scoring + manual avatar). Dead-agent salvage event for PR #496.
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`), sev1 portal hotfix for an env.ts Webpack-inline bug, backport to staging via PR #388, and TestFlight `1.0.0+4` queued at Apple. The env-var helper `requireEnv(name)` used dynamic `process.env[name]` bracket access which Webpack never inlines into client bundles — every authenticated route on `manage.homefit.studio` crashed at module load until the fix at `web-portal/src/lib/env.ts:47-63` made each helper do the literal `process.env.NEXT_PUBLIC_X` access at the call site. Diagnosis went four layers deep (Vercel env vars missing → sensitive-vars trap → Vercel redeploy → chunk byte-inspection) before finding the real cause. Main tip `1cb555e`, staging tip `b219370`, iPhone still on the dev-signed `eeaa7ea` build from `install-device.sh prod` (TestFlight install pending Apple processing).
- `docs/CHECKPOINT_2026-05-17.md` — Morning checkpoint: device QA wave on yesterday's staging tip; four visible bugs surfaced live, all fixed same-day. Photo B&W `pickPrimarySrc` parity (PR #381). Video swap-back re-hydrate (PR #382). Four-PR cache stack with URL versioning `?v=<plan.version>` as the spine (PR #385). Settings popover escaped the `backdrop-filter` containing-block trap (PR #386). Items 1-5 of test list passed cross-browser. Staging tip ended at `0329d12`, main at `9571dbf`.
...
- **Non-TrueDepth face-enrolment crop fix.** `FaceEnrolmentCameraChannel.captureFrameAndEmbed` still uses `AVMetadataFaceObject.bounds` directly without Y-flip / orientation reconciliation — so iPhone SE family users will enrol with garbage embeddings (Safe Mode self-recognition will hit cosSim ≈ 0.09 against their own face). Fix pattern: lift the `VNDetectFaceRectanglesRequest` block from `FaceEnrolmentARKitChannel.swift` (lines 463-507 area) into `captureFrameAndEmbed`, replacing the `faceBoundsNormalized` reads with the top-left-origin Y-flipped Vision rect. See PR #538's "Suggested follow-up wire" paragraph for the exact pattern. Per `docs/CHECKPOINT_2026-05-26.md`.
...
- **Apple Vision face yaw is quantized to 45° on iOS 18+ (2026-05-26):** `VNDetectFaceLandmarksRequest` (revision 3) on the streamed `CMSampleBuffer` returns yaw values from the discrete set `{0, ±45, ±90}` on iPhone 17 Pro / iOS 18.x — verified live on Carl's device via the enrolment debug HUD. Pitch comes back nil on every streamed-buffer frame. Apple's documented continuous-pose path is ARKit + TrueDepth: `ARFaceAnchor.transform` decomposed into Euler angles gives full 6-DoF head pose at the camera frame rate. The face-enrolment pipeline now branches on `ARFaceTrackingConfiguration.isSupported`: TrueDepth devices (iPhone X+) get the ARKit primary path (`FaceEnrolmentARKitChannel.swift`), non-TrueDepth devices get the Vision fallback (`FaceEnrolmentCameraChannel.swift`). Don't try to coax a continuous yaw out of Vision; it isn't there to extract.
...
- **`docs/CHECKPOINT_2026-05-26.md`** — most recent checkpoint. Face enrolment ARKit pivot wave (6 PRs `b543130` → `c347041`). ARKit primary path for TrueDepth + Vision fallback for SE family. Per-axis prompt tolerance. Yaw sign do-not-negate. Three permanent Diagnostics surfaces. Non-TrueDepth fallback crop bug still open. iPhone CHM on `c347041`. Disk space freed by Xcode DerivedData nuke (14 GB) but ~30 GB more recoverable from agent worktree build dirs.
- `docs/CHECKPOINT_2026-05-26-artifact-system-waves-4-5.md` — Autonomous overnight run landing artifact-system Waves 4 + 5 (brand-skin subscription #539, share sheet + managed email #541) + code-review follow-up (#544 with the sev1 owner-only RPC fix). Staging tip `597a600`. Outstanding Carl-side actions: Supabase Auth redirect allowlist (Wave 2) + `RESEND_API_KEY` secret + edge-function deploy (Wave 5). All 5 artifact-system waves staging-only pending the next staging → main promotion.
- `docs/CHECKPOINT_2026-05-25.md` — Safe Mode v2 polish day (six PRs + parallel enrolment work). v2-on-photos sev1 chain closed; enrolment polish phases 1 + 2 with pose gating + quality scoring. Dead-agent salvage for PR #496.
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`); sev1 portal hotfix for an env.ts Webpack-inline bug (commit `9c813f2` direct-to-main + PR #388 backport to staging); TestFlight `1.0.0+4` queued at Apple. Diagnosis lesson: Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access — bracket-notation `process.env[name]` resolves to `undefined` in the browser regardless of Vercel config. Main tip `1cb555e`, staging tip `b219370`, iPhone on dev-signed `eeaa7ea` pending TestFlight install.
- `docs/CHECKPOINT_2026-05-17.md` — Morning checkpoint: device QA wave; four bugs fixed same-day (photo B&W `pickPrimarySrc` parity #381, video swap-back re-hydrate #382, four-PR cache stack #383/#384/#385 with `?v=<plan.version>` URL versioning as the spine, settings popover containing-block escape #386). Items 1-5 of test list pass cross-browser.
