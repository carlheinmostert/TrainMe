...
- **`docs/CHECKPOINT_2026-05-28.md` — READ FIRST.** Device-QA marathon. Artifact-stacking UI redesigned from the Studio fanned-deck into a per-session **accordion** on the client/My-Workouts session lists + `/me` (3 device-feedback iteration rounds). The workout **handout** "Loading your plan" bug was root-caused after THREE wrong fixes to a one-line CSS bug (`[hidden]` defeated by author `display:flex` — the page rendered all along under a permanent loading overlay; fixed #558 via a simulator runtime repro). Publish flow made forgiving: always-enabled button + no-op toast (#557), gate pre-checks the workout so edits re-publish for free (#559), all already-published artifacts become opt-in "Out of date" free re-publish rows (#560). 12 PRs (#549–#560), all staging-only at tip `6cec276`; iPhone CHM on it. The unpublished-changes **spine** (session + exercise level) is spec'd + chipped but NOT built. Three dead-agent salvages (cold OpenCV builds stalled agents pre-commit). Immediate open item: confirm item-5-redux device QA passed.
- `docs/CHECKPOINT_2026-05-26.md` — Face enrolment ARKit pivot wave: 6 PRs landed `b543130` → `c347041` (staging tip). ARKit primary path on TrueDepth devices (iPhone X+) via `FaceEnrolmentARKitChannel.swift`; Vision fallback retained for iPhone SE family. Per-axis prompt-walker tolerance (yaw 20°, pitch 30°) replaces Manhattan sum. ARKit yaw sign convention: do NOT negate the YXZ extraction. Three permanent diagnostic surfaces under Settings → 7-tap → Diagnostics (pose HUD toggle, match-log toggle, "Test self-recognition" action). **One critical follow-up:** non-TrueDepth fallback channel still has the AVMetadata-bounds crop bug (cosSim ≈ 0.09 for SE users). Disk space: nuked Xcode DerivedData mid-session (14 GB recovered); ~30 GB more recoverable from agent worktree build dirs.
...
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`), sev1 portal hotfix for an env.ts Webpack-inline bug, TestFlight `1.0.0+4` queued at Apple. Diagnosis lesson: Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access — bracket-notation `process.env[name]` resolves to `undefined` in the browser regardless of Vercel config. Main tip `1cb555e`, staging tip `b219370`.
...
- **`docs/CHECKPOINT_2026-05-28.md`** — most recent checkpoint. Artifact-card accordion redesign (off Studio onto session lists + `/me`, 3 iteration rounds) + the handout three-fix saga (true cause: `[hidden]` defeated by `display:flex`, fixed #558) + publish-flow fixes (always-enabled button #557, gate pre-check #559, opt-in "Out of date" re-publish #560). 12 PRs #549–#560, staging-only at `6cec276`; iPhone CHM on it. Spine spec + bundle-drift cleanup chipped, not built. New gotcha: `gotcha_hidden_attr_defeated_by_display.md`.
- **`docs/CHECKPOINT_2026-05-26.md`** — Face enrolment ARKit pivot wave (6 PRs `b543130` → `c347041`). ARKit primary path for TrueDepth + Vision fallback for SE family. Per-axis prompt tolerance. Yaw sign do-not-negate. Three permanent Diagnostics surfaces. Non-TrueDepth fallback crop bug still open.
...
- `docs/CHECKPOINT_2026-05-17-pm.md` — Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`); sev1 portal hotfix for an env.ts Webpack-inline bug; TestFlight `1.0.0+4` queued at Apple. Diagnosis lesson: Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access. Main tip `1cb555e`, staging tip `b219370`.
...
- Explicit triggers: **park / log / capture / "file an issue"**. Default **one issue per distinct item**; on a burst, **"park these as one"** bundles them into a single checklist issue.
