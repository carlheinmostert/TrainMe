...
- **`docs/CHECKPOINT_2026-05-26-artifact-system-waves-4-5.md`** — most recent checkpoint. Autonomous overnight run landing artifact-system Waves 4 + 5 (brand-skin subscription #539, share sheet + managed email #541) + code-review follow-up (#544 with the sev1 owner-only RPC fix). Staging tip `597a600`. Outstanding Carl-side actions: Supabase Auth redirect allowlist (Wave 2) + `RESEND_API_KEY` secret + edge-function deploy (Wave 5). All 5 artifact-system waves staging-only pending the next staging → main promotion.
