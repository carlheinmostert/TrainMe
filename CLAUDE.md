...
- **`docs/CHECKPOINT_2026-05-11.md` — READ FIRST.** CI/CD release-train cutover. 12 PRs replaced "Claude pastes SQL in the dashboard" + "every web merge ships to prod" with a real three-tier pipeline: feature branches → `staging` → `main`. Supabase Branching live on prod project (per-PR DB previews + persistent staging branch at `vadjvkmldtoeyspyoqbx`); Vercel-Supabase integration on both web projects; persistent staging environment serving at `staging.manage.homefit.studio` + `staging.session.homefit.studio`; Flutter `ENV` flag wires branch-aware Supabase URLs; auto-tag workflow (`v{YYYY-MM-DD}.{N}` on every merge to main; `mobile-v*` on every `bump-version.sh`); 72 legacy `schema_*.sql` files archived to `supabase/archive/`; 8 new memory rules (specs-direct-to-main, use APIs not dashboards, branch-naming discipline, markdown TOC, etc.). Pipeline runs but doesn't gate yet. Carl's iPhone still on the 2026-05-04 (#223) build — no mobile install today.
