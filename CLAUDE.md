...
- **`docs/CHECKPOINT_2026-05-17-pm.md`** — most recent checkpoint. Staging→main promotion of 95 PRs (`v2026-05-11.1` → `v2026-05-17.1`); sev1 portal hotfix for an env.ts Webpack-inline bug (commit `9c813f2` direct-to-main + PR #388 backport to staging); TestFlight `1.0.0+4` queued at Apple. Diagnosis lesson: Webpack only inlines `process.env.NEXT_PUBLIC_X` for literal property access — bracket-notation `process.env[name]` resolves to `undefined` in the browser regardless of Vercel config. Main tip `1cb555e`, staging tip `b219370`, iPhone on dev-signed `eeaa7ea` pending TestFlight install.
