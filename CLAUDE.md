...
- **Logo:** Matrix of pills — a slice of a training session (3 ghost exercises tapering in from each side → 2-cycle circuit in a coral tint band → rest). Two variants: matrix-only (favicons, app icons, sub-16px badges — used wherever a wordmark would not render) + lockup (matrix + wordmark stacked, exact-width via `textLength="48"` — used wherever a wordmark renders at all, including BrandHeader, sign-in hero, web player lobby + footer, OG cards). **Wordmark colour split (locked 2026-05-22):** `homefit` is `#F0F0F5`; `.studio` (with the dot) is coral `#FF6B35` — ALWAYS, on every surface. The "matrix-only + separate `<span>homefit.studio</span>`" pattern is retired — use the lockup instead so the wordmark stays pinned to the matrix's 48-unit width. Geometry and callers in `HomefitLogo.tsx` (web portal: `HomefitLogo` matrix-only + `HomefitLogoLockup` wordmark), `homefit_logo.dart` (Flutter: same naming), and `web-player/app.js` (web player: `buildHomefitLogoSvg()` matrix-only + `buildHomefitLogoLockupSvg()` wordmark — both exist as distinct functions). Signed off at `docs/design/mockups/logo-ghost-outer.html`; wordmark split signed off at `docs/design/mockups/portal-home-cosmetic-pass.html`.
...
- **Service worker:** `web-player/sw.js` caches app shell for offline. Cache name is `homefit-player-__BUILD_SHA__` with the sentinel rewritten by `web-player/build.sh` from `VERCEL_GIT_COMMIT_SHA` at every Vercel deploy — every commit naturally busts the cache for returning visitors. Don't bump manually; the next deploy will overwrite any hand-edited value.
...
- **Logo v2 canonical** — retired heartbeat/roof. New matrix: 3 ghost greys per side (`#4B5563` / `#6B7280` / `#9CA3AF`), 2×2 circuit in coral tint band, sage rest. Matrix `viewBox="0 0 48 9.5"` + lockup `viewBox="0 -2 48 16"`. Three impls kept in sync: `HomefitLogo.tsx` / `HomefitLogoLockup`, `homefit_logo.dart`, `buildHomefitLogoSvg()` + `buildHomefitLogoLockupSvg()`.
