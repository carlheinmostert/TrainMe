...
export class AdminApi {
  ...
  async findPendingPayment(pid: string): Promise<PendingPaymentRow | null> {
    const { data, error } = await this.supabase
      ...
      .eq('id', pid)
      .maybeSingle();
    if (error) throw new Error(`[AdminApi] findPendingPayment failed: ${error.message}`);
  ...
  async applyPendingPayment(
    ...
    if (statusErr) throw new Error(`[AdminApi] pending_payments status update failed: ${statusErr.message}`);
