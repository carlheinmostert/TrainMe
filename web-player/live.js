...
(function () {
  ...
  function slugFromPath() {
    const m = String(window.location.pathname || '').match(/^\/v\/([^/]+)\/now/);
  ...
  async function poll() {
    ...
    const data = await window.HomefitApi.getLiveSessions(slug);
    ...
    if (!practiceContact) {
      practiceContact = await fetchPracticeContact(slug);
    }
