...
(function () {
  ...
  // env vars at deploy time). Falls back to prod values so the bundle
  // still works in three off-Vercel cases:
  //   1. Local file:// dev (no build step ever ran).
  //   2. The Flutter-embedded LocalPlayerServer (mirror bundle in
  //      `app/assets/web-player/`). `isLocalSurface()` short-circuits
  //      every network call below before either constant is read, so
  //      the values don't matter in this surface — but having them
  //      keeps the bundle self-contained.
  //   3. Any surface that loads api.js before config.js races to load.
  //      (config.js is a synchronous <script> in index.html so this is
  //      not expected on Vercel; the guard is belt-and-braces.)
  //
  ...
  const SUPABASE_URL = _cfg.supabaseUrl || 'https://yrwcofhovrcydootivjx.supabase.co';
  const SUPABASE_ANON_KEY = _cfg.supabaseAnonKey || 'sb_publishable_cwhfavfji552BN8X0uPIpA_pwWQ-gw3';
