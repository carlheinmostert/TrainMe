...
(function () {
  ...
  /**
   * Build a standard Supabase RPC POST request. All anon-role RPCs use
   * the same headers; extracting here eliminates the copy-paste across
   * every call site and makes future header changes a single-line edit.
   */
  function _supabasePost(rpcName, params, extraOptions) {
    return fetch(
  ...
  async function getPlanFull(planId) {
    ...
    const response = await _supabasePost('get_plan_full', { p_plan_id: planId });
    if (!response.ok) throw new Error('Plan not found');
    const payload = await response.json();
    if (!payload || !payload.plan) throw new Error('Plan not found');
  ...
  async function recordPlanOpened(planId) {
    ...
    try {
      await _supabasePost('record_plan_opened', { p_plan_id: planId });
    } catch (err) {
  ...
  async function startAnalyticsSession(planId, userAgentBucket) {
    ...
    try {
      const response = await _supabasePost('start_analytics_session', {
        p_plan_id: planId,
  ...
  async function setAnalyticsConsent(sessionId, granted) {
    ...
    if (isLocalSurface()) return;
    try {
      await _supabasePost('set_analytics_consent', {
        p_session_id: sessionId,
        p_granted: !!granted,
      });
    } catch (err) {
  ...
  async function revokeAnalyticsConsent(planId, sessionId) {
    ...
    if (isLocalSurface()) return;
    try {
      await _supabasePost('revoke_analytics_consent', {
        p_plan_id: planId,
  ...
  async function clientSelfGrantConsent(planId, kind) {
    ...
    try {
      const response = await _supabasePost('client_self_grant_consent', {
        p_plan_id: planId,
        p_kind: kind,
      });
      if (!response.ok) {
  ...
  async function getPlanSharingContext(planId) {
    if (!planId) return null;
    try {
      const response = await _supabasePost('get_plan_sharing_context', { p_plan_id: planId });
      if (!response.ok) return null;
